...
def get_error_popup_html(
    ...
    # Build contacts HTML — no leading indentation (prevents Markdown code-block parsing)
    contacts_html = ""
    for c in contacts:
        name = c.get("name", "")
        role = c.get("role", "")
        email = c.get("email", "")
        avatar = (name[:1].upper() if name else "?")
        contacts_html += f'<div class="ps-error-contact"><div class="ps-error-contact-avatar">{avatar}</div><div class="ps-error-contact-info"><div class="ps-error-contact-name">{name}</div><div class="ps-error-contact-role">{role}</div><a class="ps-error-contact-email" href="mailto:{email}">{email}</a></div></div>'

    delay_s = duration_ms / 1000.0

    # NOTE: No leading spaces on any line — indented HTML (4 spaces) is parsed as Markdown code blocks
    # and would render as raw text like in the bug screenshot.
    return f"""<style>
...
<div class="ps-error-contacts">{contacts_html}</div>
